...
inline shared_ptr<Mesh> Resources::LoadCubeMesh()
{
    if (shared_ptr<Mesh> findMesh = Get<Mesh>(L"Cube")) { return findMesh; }

	constexpr float w2 = 0.5f;
	constexpr float h2 = 0.5f;
	constexpr float d2 = 0.5f;
...
inline shared_ptr<Mesh> Resources::LoadSphereMesh()
{
	if (shared_ptr<Mesh> findMesh = Get<Mesh>(L"Sphere")) { return findMesh; }

	constexpr float radius = 0.5f; // ±¸ÀÇ ¹ÝÁö¸§
    constexpr uint32 stackCount = 20; // °¡·Î ºÐÇÒ
	constexpr uint32 sliceCount = 20; // ¼¼·Î ºÐÇÒ
	...
	constexpr float stackAngle = XM_PI / stackCount;
	constexpr float sliceAngle = XM_2PI / sliceCount;

	constexpr float deltaU = 1.f / static_cast<float>(sliceCount);
	constexpr float deltaV = 1.f / static_cast<float>(stackCount);

	// °í¸®¸¶´Ù µ¹¸é¼­ Á¤Á¡À» °è»êÇÑ´Ù (ºÏ±Ø/³²±Ø ´ÜÀÏÁ¡Àº °í¸®°¡ X)
	for (uint32 y = 1; y <= stackCount - 1; ++y)
	{
        const float phi = y * stackAngle;

		// °í¸®¿¡ À§Ä¡ÇÑ Á¤Á¡
		for (uint32 x = 0; x <= sliceCount; ++x)
		{
            const float theta = x * sliceAngle;
	...
	// ¸öÅë ÀÎµ¦½º
	constexpr uint32 ringVertexCount = sliceCount + 1;
	...
	// ³²±Ø ÀÎµ¦½º
    const uint32 bottomIndex = static_cast<uint32>(vec.size()) - 1;
    const uint32 lastRingStartIndex = bottomIndex - ringVertexCount;
	for (uint32 i = 0; i < sliceCount; ++i)
	{
		//  [last+i]-[last+i+1]
		//  |      /
